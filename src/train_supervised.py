...
def parse_option():
    ...
    parser.add_argument('--sqa_type', type=str, default='convolution')
    parser.add_argument('--residual_mode', default=True)
    ...
    parser.add_argument('--n_queries', type=int, default=20, metavar='N',
    ...
    opt.model_name = '{}_{}_lr_{}_decay_{}_trans_{}_epochs_{}'.format(opt.model, opt.dataset, opt.learning_rate,
                                                            opt.weight_decay, opt.transform, opt.epochs)
...
def main():
    ...
    # model
    backbone = create_model(opt.model, n_cls, opt.dataset)
    model = Mapping(opt, backbone, n_cls)
...
def train(epoch, train_loader, model, criterion, optimizer, opt):
    ...
    for idx, (input, target, _) in tqdm(enumerate(train_loader)):
